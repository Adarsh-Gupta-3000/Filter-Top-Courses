...
function Card({course,likedCourses,setLikedCourses}){
    ...
    function clickHandler(event){ 
        if(likedCourses.includes(course.id)){
            ...
            //phle se liked ni hai ye course
            ...
            if(likedCourses.length===0){
                ...
                setLikedCourses([...ProgressEvent, course.id]);
            }
            ...
            <div className="relative">
                ...
                <div className="absolute w-[40px] h-[40px] bg-white rounded-full right-2 bottom-[-12px] grid place-items-center ">
                    <button onClick={clickHandler}>
                        {likedCourses.includes(course.id)?(<FcLike fontSize="1.75rem "/>):(<FcLikePlaceholder fontSize="1.75rem "/>)}</button>
                </div>
            </div>
            <div className="p-4">
